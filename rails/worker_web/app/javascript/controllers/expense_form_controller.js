...
export default class extends Controller {
  ...
  amountPendingChanged(event) {
    ...
    if (amountField) {
      ...
      if (checked) {
        amountField.value = ""
        amountField.classList.add("bg-gray-100", "text-gray-400")
        amountField.placeholder = "金額未定"
      } else {
        amountField.classList.remove("bg-gray-100", "text-gray-400")
  ...
  updateFieldVisibility(item) {
    ...
    const amountPendingField = item.querySelector(".expense-amount-pending-field")
    ...
    // 掛け払いの場合のみ「金額未定」チェックを表示
    if (amountPendingField) {
      amountPendingField.classList.toggle("hidden", paymentMethod !== "credit")

      // 掛け払い以外に変更したらチェックを外してamountを復活
      if (paymentMethod !== "credit") {
        const checkbox = amountPendingField.querySelector("input[type='checkbox']")
